...
export default function ShopDetailScreen() {
  ...
  const [activeFilter, setActiveFilter] = useState('Pizza'); // All, Pizza, Burger, Pasta, Drinks
  
  const [cartItems, setCartItems] = useState<Record<string, number>>({});

  const filteredMenu = activeFilter === 'All' ? MENU_ITEMS : MENU_ITEMS.filter(i => i.category === activeFilter);
  ...
  return (
    <View style={styles.container}>
        ...
        {activeTab === 'Menu' && (
          <View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filtersScroll}>
              {['All', 'Pizza', 'Burger', 'Pasta', 'Drinks'].map(filter => (
                <TouchableOpacity
                  key={filter}
      ...
      {/* Floating Cart Bar */}
      {cartCount > 0 && (
        <TouchableOpacity style={styles.bottomCartBar} activeOpacity={0.9} onPress={() => router.push('/shop-cart')}>
